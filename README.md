...
**セマンティック（ベクトル）検索**・両者を融合した**ハイブリッド検索**を提供する。
...
- **3 モード検索**:
  - **キーワード**: PostgreSQL 全文検索（`tsvector` + GIN）。固有名詞・完全一致に強い。
  - **セマンティック**: 文の意味で検索（pgvector + HNSW、コサイン類似度）。言い換えに強い。
  - **ハイブリッド**: 上記を **RRF（Reciprocal Rank Fusion）** で融合。既定モード。
- **UX**: 検索の debounced auto-submit（300ms）、検索結果に関連度スコアをチップ表示、
  削除前の確認、作成・編集・削除後のキャッシュ即時反映。
...
> （`tsvector` / `vector` は sqlite 非対応のため）。RRF 融合は純粋関数として単体テストする。
...
- **backend** は薄いルーター（`api/`）＋ ビジネスロジック（`services/`：検索融合・埋め込み）
...
- **UI/UX**: 検索モードをタブで即切替、関連度スコアの可視化、楽観的なキャッシュ更新で
  作成・編集・削除が一覧/検索へ即反映。詳細・編集・削除を 1 つのモーダルに集約。
