...
```bash
mise install            # Node 24 / Python 3.14 / uv / pnpm 11.5.2 を取得
...
- **UI/UX**: 検索モードをタブで即切替、Enter 実行で無駄な API 呼び出しを抑制、検索条件（キーワード・
  モード）を URL クエリに同期（リロード・共有・戻る/進むで復元）、関連度を 5 段階で可視化、楽観的な
  キャッシュ更新で作成・編集・削除が一覧/検索へ即反映。詳細・編集・削除を 1 つのモーダルに集約。
