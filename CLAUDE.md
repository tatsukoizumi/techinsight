...
- **Infra:** Docker Compose（`db` / `backend` / `frontend` / 一回限りの `migrator` サービス）。Docker イメージ内でも **mise** で言語ランタイムを管理し、ホストとバージョン定義を一元化。
...
mise install                                   # Node 24.16.0 を入れる（mise.toml の定義に従う）
corepack enable                                # Node 同梱の corepack で pnpm 11.5.2 を有効化
pnpm install                                   # workspace 全体（root + frontend）
# ↑3 つを `mise run setup` でまとめて実行も可（mise install を別途やった後）
...
2. **ランタイム / ツールは mise で管理する**（ホスト・Docker 共通）— Node / Python のバージョンは `mise.toml` で single source of truth。Dockerfile 内でも `apt install python3` のような直接インストールはせず、`mise install` を使う。
